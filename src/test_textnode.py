...
class TestTextNode(unittest.TestCase):
    ...
    def test_no_url_in_link_or_image(self):
        ...
        self.assertEqual(str(cm.exception), f'url is needed in the text type {TextType.LINK}, add an url please.')

        with self.assertRaises(ValueError) as cm:
            TextNode("This is a text node", TextType.IMAGE)
        self.assertEqual(str(cm.exception), f'url is needed in the text type {TextType.IMAGE}, add an url please.')
    ...
    def test_url_shouldnt_be_in_text_types_that_dont_need_it(self):
        ...
        self.assertEqual(str(cm.exception), f"url doesn't work with the text type {TextType.TEXT_BOLD}, use {TextType.IMAGE} or {TextType.LINK}")

        with self.assertRaises(ValueError) as cm:
            TextNode("This is a text node", TextType.TEXT_NORMAL, "https://blank.page")

        self.assertEqual(str(cm.exception), f"url doesn't work with the text type {TextType.TEXT_NORMAL}, use {TextType.IMAGE} or {TextType.LINK}")

        with self.assertRaises(ValueError) as cm:
            TextNode("This is a text node", TextType.TEXT_ITALIC, "https://blank.page")

        self.assertEqual(str(cm.exception), f"url doesn't work with the text type {TextType.TEXT_ITALIC}, use {TextType.IMAGE} or {TextType.LINK}")

    def test_repr(self):
        node = TextNode("This is a text node", TextType.LINK, "https://blank.page")
        self.assertEqual("TextNode(This is a text node, link, https://blank.page)",repr(node))
