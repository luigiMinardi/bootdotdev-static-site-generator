...
class TextNode:
    def __init__(self, text: str, txt_type: TextType, url: None | str = None) -> None:
        ...
        if self.url and (self.text_type is not TextType.LINK and self.text_type is not TextType.IMAGE):
            ...
            '''
            raise ValueError(f"url doesn't work with the text type {self.text_type}, use {TextType.IMAGE} or {TextType.LINK}")

        if not self.url and (self.text_type is TextType.LINK or self.text_type is TextType.IMAGE):
            '''
            what is here is either a LINK or an IMAGE but it DOESNT have an URL
            thus should raise an error
            '''
            raise ValueError(f"url is needed in the text type {self.text_type}, add an url please.")
